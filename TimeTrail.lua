...
function displayTimeNearMouse()
    -- 1. STATE VARIABLES (Cache)
    -- We store the computed text/color here so the mouse doesn't have to calculate it
    local currentBatteryColor = normalColor
    local currentStyledText   = nil
    local hoursTextCanvas     = hs.canvas.new(hs.geometry.rect(0, 0, canvasWidth, canvasHeight))

    -- 2. HEAVY TASKS (Run rarely)

    -- Task A: Check Battery (Run every 60s)
    local function updateBatteryState()
        ...
        if batteryPercentage <= batteryThreshold and not isCharging then
            currentBatteryColor = lowBatteryColor
        else
            currentBatteryColor = normalColor
        end
    end

    -- Task B: Generate Text Object (Run every 1s)
    -- We generate the styled text object here so the mouse loop simply grabs it
    local function preRenderText()
        local formatString = is24HourFormat and "%H" or "%I"
        local newText = hs.styledtext.new(os.date(formatString), {
        ...
        currentStyledText = newText

        -- Update the canvas content immediately
        hoursTextCanvas[1] = {
            type = "text",
            text = currentStyledText,
        }
    end
    ...
    -- Timers for heavy tasks
    local batteryTimer = hs.timer.new(60, updateBatteryState)
    local textUpdateTimer = hs.timer.new(1, preRenderText) -- Updates clock text every second
    batteryTimer:start()
    textUpdateTimer:start()

    -- 3. LIGHTWEIGHT TASKS (Run constantly on Mouse Move)
    ...
    local function getHoursTextPosition(mousePosition, angle)
        -- Standard Trig Math (Very fast, safe to do in loop)
    ...
    local function fadeOutText()
        local currentAlpha = hoursTextCanvas:alpha()
        if currentAlpha > 0 then
            hoursTextCanvas:alpha(currentAlpha - fadeOutAlphaStep)
            hs.timer.doAfter(fadeOutStep, fadeOutText)
        else
            hoursTextCanvas:hide()
        end
    end
    ...
    -- THE CRITICAL LOOP
    -- This now does ZERO hardware polling. It only does math and moves the UI.
    local updateOnMouseMove = hs.eventtap.new({ hs.eventtap.event.types.mouseMoved }, function(event)
        ...
        -- We move the existing canvas (cheap)
        hoursTextCanvas:topLeft(hoursTextPosition)

        -- Management
        hoursTextCanvas:show()
        hoursTextCanvas:alpha(1)
        hideTextTimer:stop()
        hideTextTimer:start()
    ...
    return { updateOnMouseMove, hideOnKeyDown, batteryTimer, textUpdateTimer }
end

-- Start the watcher
local watchers = displayTimeNearMouse()

-- Screen watcher to pause/resume (unchanged)
function screenWatcherCallback(eventType)
    if eventType == hs.caffeinate.watcher.screensDidUnlock then
        if not watchers[1]:isEnabled() then watchers[1]:start() end
    elseif eventType == hs.caffeinate.watcher.screensDidLock then
        if watchers[1]:isEnabled() then watchers[1]:stop() end
    end
end
