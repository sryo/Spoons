...
function checkForHotCorner(x, y)
    ...
    local left, right = f.x, f.x + f.w
    local top, bottom = f.y, f.y + f.h
    return (x <= left + 4 and y <= top + 4 and "topLeft")
        or (x >= right - 4 and y <= top + 4 and "topRight")
        or (x >= right - 4 and y >= bottom - 4 and "bottomRight")
        or (x <= left + 4 and y >= bottom - 4 and "bottomLeft")
end
