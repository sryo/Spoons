...
-- Check if there's an adjacent screen at the cursor position on the given edge
local function hasAdjacentScreenAt(edge, x, y, currentFrame)
    local screens = hs.screen.allScreens()
    local tolerance = 10  -- pixels of tolerance for edge alignment

    for _, s in ipairs(screens) do
        local f = s:fullFrame()
        -- Skip the current screen
        if f.x == currentFrame.x and f.y == currentFrame.y and f.w == currentFrame.w and f.h == currentFrame.h then
            goto continue
        end

        if edge == "left" then
            -- Check if screen is directly to the left and cursor Y is within its range
            if math.abs((f.x + f.w) - currentFrame.x) < tolerance then
                if y >= f.y and y <= f.y + f.h then
                    return true
                end
            end
        elseif edge == "right" then
            -- Check if screen is directly to the right and cursor Y is within its range
            if math.abs(f.x - (currentFrame.x + currentFrame.w)) < tolerance then
                if y >= f.y and y <= f.y + f.h then
                    return true
                end
            end
        elseif edge == "top" then
            -- Check if screen is directly above and cursor X is within its range
            if math.abs((f.y + f.h) - currentFrame.y) < tolerance then
                if x >= f.x and x <= f.x + f.w then
                    return true
                end
            end
        elseif edge == "bottom" then
            -- Check if screen is directly below and cursor X is within its range
            if math.abs(f.y - (currentFrame.y + currentFrame.h)) < tolerance then
                if x >= f.x and x <= f.x + f.w then
                    return true
                end
            end
...
mouseTunnelHandler = hs.eventtap.new({ hs.eventtap.event.types.mouseMoved, hs.eventtap.event.types.leftMouseDragged },
    function(e)
        ...
        -- In "all" mode, skip edges where an adjacent monitor exists at cursor position
        if WRAP_MODE == "all" and hasAdjacentScreenAt(edge, pos.x, pos.y, frame) then
            return false
        end
