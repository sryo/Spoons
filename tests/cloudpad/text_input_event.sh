#!/usr/bin/env bash
# CloudPad: POST /events with a text payload types the string into the focused
# app via hs.eventtap.keyStrokes. Verified through TextEdit's AXValue.
set -euo pipefail
...
# 3. Post a text event with "Hello".
...
# 5. TextEdit's AXValue should contain "Hello" (substring match; TextEdit may
...
wait_until '
  ...
  ")
  echo "$cur" | grep -q "Hello"
' 3 "TextEdit contains Hello"

echo "PASS: cloudpad_text_input_event"
