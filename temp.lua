...
local cbinspect             = function(...)
    ...
    local timestamp = os.date("%F %T" .. string.format("%-5s", ((tostring(date):match("(%.%d+)$")) or "")),
        math.floor(date))
...
local buildChoicesForObject = function(obj)
    ...
    for k, v in fnutils.sortByKeys(aav) do
        ...
        elseif getmetatable(v) == axmetatable then
            ...
            ", Description: " .. tostring(v.AXValueDescription or v.AXDescription or v.AXRoleDescription)
        else
        ...
        if objIsAXUIElement and obj:isAttributeSettable(k) then
            ...
            ((#entry.subText > 0) and ", s" or "S") .. "ettable (hold down ⌘ when selecting to show setter form)"
            entry.settable = true
        end
        ...
        if actions then
            ...
            for i, v in ipairs(actions) do
                table.insert(choices, {
                    text        = "Action: " .. v,
                    subText     = (obj:actionDescription(v) or "no description") ..
                    ", hold down ⌘ when selecting to perform",
                    action      = v,
                    cmdAddition = ":do" .. v .. "()",
                    cmdNoAdd    = true,
                })
...
_chooser                    = chooser.new(chooserCallback):searchSubText(true):showCallback(showingChooser):hideCallback(
hidingChooser)
